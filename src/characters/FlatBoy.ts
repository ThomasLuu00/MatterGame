class FlatBoy extends Phaser.Physics.Matter.Sprite {
    ...
    constructor(world: Phaser.Physics.Matter.World, x = 0, y = 0) {
        super(world, x, y, 'idle', 'flatboy_idle_01', { render: { sprite: { xOffset: 70, yOffset: 0 } } });
        this.shapes = Object.assign(
            {},
            this.scene.cache.json.get('idle_shapes'),
            this.scene.cache.json.get('dead_shapes'),
        );
        this.addAnimation();
    ...
    private addAnimation = () => {
        const animationCallBack = (
            ...
            const sx = gameObject.x;
            const sy = gameObject.y;

            //let shape = this.shapes[frame.textureFrame];
            //gameObject.setBody(shape);

            //gameObject.setPosition(sx, sy);
            const cx = gameObject.centerOfMass.x;
            const cy = gameObject.centerOfMass.y;
            this.setOrigin(cx - 0.05, cy + 0.31);
        };
